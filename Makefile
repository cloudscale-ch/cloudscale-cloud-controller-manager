...
# Version metadata - TAG takes precedence, git as fallback
VERSION ?= $(TAG)
GIT_COMMIT ?= $(shell git rev-parse HEAD 2>/dev/null || echo "unknown")
BUILD_DATE ?= $(shell date -u +'%Y-%m-%dT%H:%M:%SZ')

# Set version metadata fields in k8s.io/component-base/version
...
.PHONY: build
build: ## Build CCM binary.
	CGO_ENABLED=0 go build -ldflags '$(LDFLAGS)' -trimpath -o bin/cloudscale-cloud-controller-manager \
