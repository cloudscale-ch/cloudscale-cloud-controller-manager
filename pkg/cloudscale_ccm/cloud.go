...
// goroutines to perform housekeeping or run custom controllers specific to the
// cloud provider. Any tasks started here should be cleaned up when the stop
// channel closes.
...
// implemented by external cloud providers. Implementing InstancesV2 is
// behaviorally identical to Instances but is optimized to significantly
// reduce API calls to the cloud provider when registering and syncing nodes.
...
// supported, false otherwise. DEPRECATED: Zones is deprecated in favor of
// retrieving zone/region information from InstancesV2. This interface will not
// be called if InstancesV2 is enabled.
